...
int main(int argc, char *argv[])
{
  ...
  /* init line */
  // memset(user, 0x0, MAX_MSG);
  // memset(pass, 0x0, MAX_MSG);

  // read_line(newSd, user);
  // read_line(newSd, pass);
  ...
  while (1)
  {
    ...
    if (strcmp(user, USERNAME) != 0 || strcmp(pass, PASSWORD) != 0) {
      write(newSd, "exit", MAX);
      // close(newSd);
    } else {
      write(newSd, "allowed", MAX);
    }
    ...
    while (read_line(newSd, line) != ERROR)
    {
      ...
      string result = as8res(in);

      int n = result.length();

      printf("Sending:\n%s\n", result.c_str());
      write(newSd, result.c_str(), MAX);
