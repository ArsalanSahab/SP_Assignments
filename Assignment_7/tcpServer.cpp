...
int main(int argc, char *argv[])
{
  ...
  while (1)
  {
    ...
    memset(userName, 0x0, MAX_MSG);
    memset(userPass, 0x0, MAX_MSG);

    read_line(newSd, userName);
    read_line(newSd, userPass);
    
    if (strcmp(userName, USERNAME) != 0 || strcmp(userPass, PASSWORD) != 0) {
      write(newSd, "Invalid", 1000);
      // close(newSd);
    } else {
      write(newSd, "Valid", 1000);
    }
    ...
    while (read_line(newSd, line) != ERROR)
    {
      ...
      string result = simCommands(count, command);

      int n = result.length();
      char * res_ch;

      printf("Sending:\n%s\n", result.c_str());
      write(newSd, result.c_str(), 1000);
